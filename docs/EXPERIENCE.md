...
165. **存全 1 常量到 `extern u16 数组[]` 元素 (`arr[i] = -1;`) 走 GCC2 窄化 store 的 ldrh/orr/strh 展开; 一切可树折叠的同语义写法折叠成直接 strh —— orr 是分配承重指令** (2026-09-06, 案例 sub_80526A0)。
     ...
     关联: 经验 29/96/118/150。
166. **宽类型截断暂存 (如 `long long dest; dest = 指针运算; *ptr = dest;`) 是分配承重写法; bytecmp 对含 bl 候选的伪影有两层** (2026-09-06, 案例 sub_804FA94)。
     ① 跳转路径的目标地址计算赋回 `*ptr` 时, 若直写 `*ptr = 表值 + 基址` 分配错位, 把结果先存进
     **比 u32 宽的局部** (long long) 再赋 `*ptr` 可复现目标 home —— 语义仍是"算好再存", 无数据流
     欺骗, 且换 u32 局部就不行 (宽类型强制独立 home + 截断)。② bytecmp 伪影两层: (a) bl 差 4B/条
     —— 候选侧过部分链接 bl 被补丁到桩地址, target 侧 objcopy 直出是原始占位 (经验 156 的
     "4×bl=8B" 机制, fncheck 才是权威); (b) 函数桩地址给太远 (如 0x0804E76C 距 .text 0 超 ±4MB)
     时 ld 会插 veneer, mine.bin 尾部多 32B —— 都是装置伪影不是代码差异, 看 objdump 段大小即可分辨。
     ③ 比较类表达式的操作数序勿翻: `n > i`/`v > 0x1FF` 对应 `cmp r7,r4`/`cmp r1,sb`, 翻转成
     `i < n` 可能变 `cmp r4,r7; blo` 字节错位。④ 未初始化局部在目标里同样无初始化指令时, C 也
     保持无初值 (零循环路径读寄存器残值 = 原始行为), 别"修"它。
     关联: 经验 156/164, 经验 29。
