...
{
    internal class SoundDumper
    {
        private static int MPlayTableAdr = 0x086B0E44;
        private static int NumMPlayers = 5;
        private static int SongTableAdr = MPlayTableAdr + NumMPlayers * 12;
        private static int NumSongs = 285;

        private static int VoiceGroupEndAdr = 0x086B0D34;

        private static SortedDictionary<int, string> soundInfo = new SortedDictionary<int, string>();
        private static byte[] rom = File.ReadAllBytes("../../baserom.gba");
        private static List<int> voicegroups = new List<int>();

        private static Dictionary<int, string> MML = new Dictionary<int, string>()
        {
            {0x80, "W00" },
            {0x81, "W01" },
            {0x82, "W02" },
        ...
        private static Dictionary<string, bool> SongsWithTrackAlignment = new Dictionary<string, bool>()
        {
        };

        private static byte ReadU8(int ofs)
        {
            Debug.WriteLine("ofs = 0x{0:X8}", ofs);
            if (ofs > 0x08000000)
                ofs -= 0x08000000;
        ...
        private static ushort ReadU16(int ofs)
        {
            if (ofs > 0x08000000)
                ofs -= 0x08000000;
        ...
        private static int ReadU32(int ofs)
        {
            if (ofs > 0x08000000)
                ofs -= 0x08000000;
        ...
        private static string PrintMPlayTable(int ofs)
        {
            ...
            for (int i = 0; i < NumMPlayers; i++)
            {
                ...
                sb.AppendFormat("\tmusic_player gUnk_{0:X8}, gUnk_{1:X8}, {2}, {3}\n", playInfo, trackInfo, unk1, unk2);
                ofs += 12;
            }
        ...
        private static void DumpProgrammableWaveSample(int ofs, int len)
        {
            ...
            sb.Append("\t.align 2\n");
            sb.AppendFormat("\t.global gUnk_{0:X8}\n", ofs);
            sb.AppendFormat("gUnk_{0:X8}:\n", ofs);
        ...
        private static void DumpDirectSoundSample(int ofs)
        {
            if (ofs == 0) return;
            if (soundInfo.ContainsKey(ofs)) return;
            ...
            sb.Append("\t.align 2\n");
            sb.AppendFormat("\t.global DirectSoundData_{0:X8}\n", ofs);
            sb.AppendFormat("DirectSoundData_{0:X8}:\n", ofs);
        ...
        private static string PrintVoiceGroup(int ofs, int len, int id)
        {
            var sb = new StringBuilder();
            ...
            while (i < len)
            {
                ...
                switch (type)
                {
                    ...
                    case 0x09: //voice_square_1_alt
                        {
                            //key, unk, sweep, duty_cycle, attack, decay, sustain, release
                            var key = ReadU8(ofs + i++);
                            var unk = ReadU8(ofs + i++);
                            ...
                            {
                                case 0x01: //voice_square_1
                                    sb.AppendFormat("\tvoice_square_1 {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\t@{8:X8}\n", key, unk, sweep, duty_cycle, attack, decay, sustain, release, tOfs);
                                    break;
                                case 0x09: //voice_square_1_alt
                                    sb.AppendFormat("\tvoice_square_1_alt {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\t@{8:X8}\n", key, unk, sweep, duty_cycle, attack, decay, sustain, release, tOfs);
                                    break;
                            }
                        }
                        break;
        ...
        private static string PrintSongTrack(int idx, int ofs, int len, int song_ofs, string name)
        {
            var sb = new StringBuilder();
        ...
        private static string PrintSong(int ofs, int id)
        {
            var sb = new StringBuilder();
            var sbt = new StringBuilder();
            var currName = String.Format("song_{0:d4}", id);
            var realName = currName;
            if (soundInfo.ContainsKey(ofs))
            {
                int pos = soundInfo[ofs].IndexOf(".global song");
                realName = soundInfo[ofs].Substring(pos + 8, 9);
            }
            ...
            // if (id > 0)
            {
                ...
                {
                    sb.AppendFormat("\t.word\tgUnk_{0:X8}\t\t@ voicegroup/tone\n\n", tone);
                }
                else
                {
        ...
        private static string PrintSongTable(int ofs)
        {
            var sb = new StringBuilder();
            ...
            for (int i = 0; i < NumSongs; i++)
            {
                ...
                sb.AppendFormat("\tsong song_{0:d4}, {1}, {2}\n", i, ms, me);
        ...
        private static void Main(string[] args)
        {
            ...
            foreach (var adr in soundInfo)
            {
                ...
                if (adr.Value.Contains(".global song"))
                {
                    ...
                    string curr = adr.Value.Substring(pos + 8, 9);
