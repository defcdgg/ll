...
#define DmaSet(dmaNum, src, dest, control)                                                                                            \
    do                                                                                                                                \
    {                                                                                                                                 \
        _Static_assert(_Alignof(src)                                                                                                  \
                           >= __builtin_choose_expr(__builtin_constant_p(control), ((control) & (DMA_32BIT << 16)) ? 4 : 2, 2),       \
                       "source potentially unaligned");                                                                               \
        _Static_assert(_Alignof(dest)                                                                                                 \
                           >= __builtin_choose_expr(__builtin_constant_p(control), ((control) & (DMA_32BIT << 16)) ? 4 : 2, 2),       \
                       "destination potentially unaligned");                                                                          \
        DmaSetUnchecked(dmaNum, src, dest, control);                                                                                  \
    } while (0)
#else
#define DmaSet(dmaNum, src, dest, control) DmaSetUnchecked(dmaNum, src, dest, control)
#endif
...
#define DMA_FILL_UNCHECKED(dmaNum, value, dest, size, bit)                                                                            \
    {                                                                                                                                 \
        ...
        DmaSet(dmaNum, &tmp, dest,                                                                                                    \
               (DMA_ENABLE | DMA_START_NOW | DMA_##bit##BIT | DMA_SRC_FIXED | DMA_DEST_INC) << 16 | ((size) / (bit / 8)));            \
    }
...
#define DMA_COPY_UNCHECKED(dmaNum, src, dest, size, bit)                                                                              \
    DmaSet(dmaNum, src, dest, (DMA_ENABLE | DMA_START_NOW | DMA_##bit##BIT | DMA_SRC_INC | DMA_DEST_INC) << 16 | ((size) / (bit / 8)))
